...
export default function Careers() {
  ...
  useEffect(() => {
    ...
    setStatus('loading')
    fetch(`${API_BASE}/api/vacancies/`, { signal: ctrl.signal })
      .then((r) => {
        if (!r.ok) throw new Error(`HTTP ${r.status}`)
        return r.json()
      })
      .then((data: ApiVacancy[]) => {
        // Map slug -> id so the existing Vacancy type/contract is preserved.
        setVacancies(data.map(({ slug, ...rest }) => ({ id: slug, ...rest })))
        setStatus('ready')
      })
      .catch((err) => {
  ...
  return (
    ...
    >
      <div className="relative mx-auto max-w-7xl px-5 md:px-6 lg:px-10">
        ...
        {/* Grid */}
        {status === 'loading' ? (
          <div className="mt-12 flex items-center justify-center gap-3 py-16 text-neutral-500">
            <Loader2 className="h-5 w-5 animate-spin text-brand-600" />
            <span className="text-sm font-medium">Загружаем вакансии…</span>
          </div>
        ) : status === 'error' ? (
