...
import { vacancies, type Vacancy } from '../data/content'
import { useModal } from '../context/ModalContext'

// Map the icon name stored in content.ts to a real lucide component
    ...
    >
      <div className="relative mx-auto max-w-7xl px-5 md:px-6 lg:px-10">
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {vacancies.map((v, i) => (
            <VacancyCard key={v.id} vacancy={v} index={i} reduce={!!reduce} onApply={open} />
          ))}
        </div>
      </div>
    </section>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        {/* CTA — opens the existing contact modal. NOTE: open() takes no payload,
            so role context isn't prefilled; wire a payload here if the modal gains one. */}
        <button
          type="button"
          onClick={onApply}
