...
// === Отправка в Telegram ===
// Значения берутся из .env (VITE_TELEGRAM_BOT_TOKEN / VITE_TELEGRAM_CHAT_ID).
// Внимание: VITE_-переменные вшиваются в браузерный бандл и видны в коде сайта —
// это особенность фронтового варианта без серверной функции.
...
// Собирает выбранные направления + ответы по ним + контакты в текст для бота
function buildLeadMessage({ selected, answers, name, contact, phone }: LeadPayload): string {
  const names = selected.map(dirLabel).join(", ") || "—";
  const lines = selected
    .filter((id) => QUESTIONS[id])
    .map((id) => {
      const a = answers[id];
      const ans = Array.isArray(a) ? a.join(", ") : a || "—";
      return `• <b>${dirLabel(id)}</b> — ${QUESTIONS[id].q} <b>${esc(ans)}</b>`;
    });
  const parts = ["🆕 <b>Новая заявка с сайта</b>", "", `Направления: ${names}`];
  if (lines.length) parts.push("──────────", ...lines);
  if (selected.includes("unsure")) parts.push("⚠️ Клиент выбрал «Не знаю — помогите»");
  parts.push("──────────", `👤 Имя: ${esc(name)}`, `✈️ Telegram/email: ${esc(contact)}`, `📞 Телефон: ${esc(phone)}`);
  return parts.join("\n");
}
...
export default function ContactForm({ initialSelected = [] }: { initialSelected?: string[] }) {
  ...
  const questionDirs = DIRECTIONS.filter((d) => selected.includes(d.id) && QUESTIONS[d.id]).map((d) => d.id);
  const totalSteps = 1 + questionDirs.length + 1;
  const contactsScreen = 1 + questionDirs.length;
  const onSelection = screen === 0;
...
function SuccessView({ selected, unsure }: { selected: string[]; unsure: boolean }) {
  ...
  return (
    <div style={{ height: "100%", minHeight: 380, display: "flex", flexDirection: "column",
      ...
      <h3 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>Заявка отправлена!</h3>
